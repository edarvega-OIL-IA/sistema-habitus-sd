...
export default function CarritoPage() {
  ...
  if (!cargado) {
    return <div className="min-h-screen bg-[#ededed] flex items-center justify-center"><p className="text-sm text-gray-400">Cargando carrito...</p></div>
  }
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        {items.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-lg p-12 text-center">
            <p className="text-sm text-gray-400 mb-3">Todavía no agregaste nada al carrito.</p>
          ...
          <>
            ...
            <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100">
              {items.map(it => (
                <div key={it.articuloId} className="p-4 flex gap-3 items-center">
                  <div className="w-16 h-16 rounded-lg bg-[#f5f5f4] shrink-0 flex items-center justify-center overflow-hidden">
                    {it.imagenUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    {it.marca && <p className="text-[10px] text-gray-400 uppercase tracking-wide">{it.marca}</p>}
                    <p className="text-sm font-medium text-[#3c3c3b] leading-snug">{it.nombreBase}</p>
                    {it.sabor && <p className="text-xs text-gray-400">{it.sabor}</p>}
                    <p className="text-sm font-bold text-[#3c3c3b] mt-1">{fmt(it.precio)}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <button onClick={() => quitar(it.articuloId)} className="text-gray-300 hover:text-red-500 transition-colors">
                    ...
                    <div className="flex items-center border border-gray-300 rounded-lg">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center text-gray-500 hover:text-[#00a19a] disabled:opacity-30"
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="w-7 text-center text-sm font-medium text-[#3c3c3b]">{it.cantidad}</span>
                      <button
                        onClick={() => actualizarCantidad(it.articuloId, it.cantidad + 1)}
            ...
            <div className="flex justify-between items-center mt-4">
              <button onClick={vaciar} className="text-xs text-gray-400 hover:text-red-500">Vaciar carrito</button>
