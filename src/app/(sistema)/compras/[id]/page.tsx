...
export default function ComprasEditarPage() {
  ...
  async function cargarDatos() {
    ...
    try {
      const [provRes, transRes, artRes, tasasRes, ordenRes] = await Promise.all([
        ...
        supabase.from('ordenes_compra').select(`
          ...
          orden_compra_items (
            ...
            precio_unitario_con_iva, precio_unitario_sin_iva, descuento_pct, subtotal, es_ajuste_redondeo,
      ...
      const itemsCargados: ItemOrden[] = itemsReales.map((it: any) => {
        // Si ya tenemos el precio tal cual se cargó, usarlo directo — más
        // confiable que reconstruirlo desde precio_unitario_sin_iva (que
        // puede estar mal si la orden se creó antes de centralizar el costo real
        // en RECUPERA_IVA_COMPRAS (src/lib/config.ts).
        let precioConIva: number
        if (it.precio_unitario_con_iva != null) {
          precioConIva = it.precio_unitario_con_iva
        } else {
          const tasa = tasas.find(t => t.id === it.articulos?.tasa_iva_id)
          const divisor = tasa ? 1 + tasa.porcentaje / 100 : 1.21
          precioConIva = Math.round(it.precio_unitario_sin_iva * divisor * 100) / 100
        }
        return {
          ...
          // No existe columna descuento_monto en BD — se deriva del % guardado
          // solo para mostrarlo en pantalla; el subtotal real sigue siendo el
          // que ya está guardado (it.subtotal), no uno recalculado acá.
          descuento_monto: it.cantidad_facturada * precioConIva * ((it.descuento_pct || 0) / 100),
  ...
  function agregarArticulo(art: Articulo) {
    ...
    setItems(prev => [...prev, {
      ...
      precio_unitario: precio, descuento_pct: 0, descuento_monto: 0, subtotal: precio,
    }])
    ...
    try {
      ...
      const { error: itemsInsertError } = await supabase.from('orden_compra_items').insert(
        itemsConFlete.map(it => ({
          ...
          precio_unitario_con_iva: it.precio_unitario,
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-200">
            ...
            <div>
              ...
              <input type="date" value={fechaFactura} onChange={e => setFechaFactura(e.target.value)}
                ...
                disabled={esAnulada}
                className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#00a19a] disabled:bg-gray-50 ${errFechaFactura && !fechaFactura ? 'border-red-500' : 'border-gray-300'}`} />
              {errFechaFactura && !fechaFactura && (
                <p className="text-xs text-red-500 mt-1">Fecha fuera de rango, revisá el año</p>
              )}
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="date" value={fechaOrden} onChange={e => setFechaOrden(e.target.value)}
              ...
              disabled={esAnulada}
              className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#00a19a] disabled:bg-gray-50 ${errFechaOrden && !fechaOrden ? 'border-red-500' : 'border-gray-300'}`} />
          ...
          <div>
            ...
            <input type="date" value={fechaRecepcion} onChange={e => setFechaRecepcion(e.target.value)}
              ...
              disabled={esAnulada}
              className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#00a19a] disabled:bg-gray-50 ${errFechaRecepcion && !fechaRecepcion ? 'border-red-500' : 'border-gray-300'}`} />
