...
**Última actualización:** 09/08/2026 (continuación 3) — **Mínimo de compra cambiado de por-producto a por-rubro** en "Barras de proteína" y "Geles" (probado en local, funcionando), agregado texto de ayuda en el checkout aclarando quién debe figurar como "Nombre y apellido" (quien retira, DNI a pedir en el local), y confirmado un tercer caso (#1498) del patrón ya conocido de fiscalización que falla en el primer intento y se resuelve con "Reintentar".
**Estado general:** 🟢 En producción. Vitrina con circuito de compra completo (Bloque 12), diseño/accesibilidad 20/20 (Bloque 14), verificada en celular real sin bugs de touch (Bloque 15), y mínimo de compra por rubro aplicado en el frontend (Bloque 16).
**Próxima acción concreta:** llevar la regla del mínimo de compra por rubro también a la validación del lado del servidor/checkout (hoy solo existe en el frontend). Después, seguir con fotos de los rubros recién migrados y el resto de pendientes de Vitrina. Ver Bloque 16, sección 25.
...
1. Llevar el mínimo de compra por rubro (Bloque 16) a la validación real del servidor/checkout — hoy solo existe como aviso visual y bloqueo de botón en el frontend.
